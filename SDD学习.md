...
注意：在使用的时候也要将需求讲清楚，skill只是一个用于提示质量的规范，如果不讲清楚，AI很可能会越级，比如使用openspec\
:propose时，它可能不仅仅只生成文档，还有可能会修改项目。

相较于BMAD而言更适合小需求的迭代而不是从零开始启动一个项目，将BMAD和openspec相结合使用，通过BMAD来生成需求文档和PRD和大致框架，后续可以使用openspec来对需求迭代
