...
\<required\_reading>
Read all files referenced by the invoking prompt's execution\_context before starting.
?:execution\_context哪里来的，去哪里找，刚开始初始化时项目中没有这个怎么办？在这个skill的SKILL.md文档中
我只知道在.planning\phases\01-基础引擎\01-01-PLAN.md中有execution\_context，应该是在这里找，但是agent是如何知道在这个文件中有execution\_context的？难道是按序读取文件然后读到的？还是因为log中有写？是不是只有在workflow中需要去读取这个execution\_context？如果只调用skill不用读取？
\</required\_reading>
...
?:section_manifest在buildSectionManifestField函数中生成，在init.cts中定义的，顾名思义是在初始化时调用的，section_manifest中有included字段
...
?:通过脚本获取到了AGENT\_SKILLS\_RESEARCHER这些变量
同时也获取到了json数据，然后下面就从json中获取到想要的字段变量，后续的工作流中直接使用这些变量
